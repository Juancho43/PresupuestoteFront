...
export class SalaryService implements ICrudeable<Salary,SalaryRequest> {
  ...
  create(entity: SalaryRequest): Observable<ApiResponse<Salary>> {
    return this.http.post<ApiResponse<Salary>>(environment.apiUrlV1 + salaryEndpoint.create, entity) .pipe(
      tap(() => {
  ...
  update(entity: SalaryRequest): Observable<ApiResponse<Salary>> {
    return this.http.put<ApiResponse<Salary>>(environment.apiUrlV1 + salaryEndpoint.update.replace(':id',entity.id!.toString()), entity) .pipe(
      tap(() => {
        this.notification.showSuccessNotification();
      }),
      catchError(() => {
  ...
  delete(id: number): Observable<ApiResponse<Salary>> {
    return this.http.delete<ApiResponse<Salary>>(environment.apiUrlV1 + salaryEndpoint.delete.replace(':id', id.toString())) .pipe(
      tap(() => {
        this.notification.showSuccessNotification();
      }),
      catchError(() => {
