...
export class BudgetFormComponent {
  ...
  onDelete() {
    const result = this.confirmationService.openDialog('¿Está seguro de que desea eliminar este presupuesto?');
    result.afterClosed().subscribe(result =>{
      if (result) {
        this.budgetService.delete(this.budgetResource.value()?.data?.id!).subscribe({
          next: () => {
            this.confirmationService.navigateTo('/budget')
          }
        });
      }
    })
